...
if __name__ == "__main__":
    ...
    if args.meta_calibration == "learnable_l2":
        ...
        meta_optimizer = torch.optim.Adam(learnable_regularization)
    elif args.meta_calibration == "onehot":
        learnable_regularization = [
            nn.Parameter(torch.zeros_like(e)) for e in net_fc.parameters()
        ]
        meta_optimizer = torch.optim.Adam(learnable_regularization)
    ...
    with tqdm.tqdm(total=num_epochs) as pbar_epochs:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_set_ece[epoch]=train_ece
            val_set_ece[epoch]=val_ece

            sq_training_set_loss.append(train_loss)
            sq_val_set_loss.append(val_loss)
            sq_test_set_loss.append(test_loss)
            sq_val_set_err.append(1 - val_acc)
            sq_train_set_ece.append(train_ece)
            sq_val_set_ece.append(val_ece)
            ...
            if args.meta_calibration == "onehot":
                # only store the overall scaling to avoid too large json file
                experiment_update_dict["learn_reg_vals"] = learnable_regularization[
                    -1
                ].tolist()
    ...
    # df = pd.DataFrame(sq_training_set_loss)
    #
    # df.to_csv("training_set_loss.csv", index=False, encoding="utf-8")
    #
    # cpu_tensor_list = [tensor.cpu() for tensor in sq_val_set_loss]
    # df = pd.DataFrame(cpu_tensor_list)
    #
    # df.to_csv("val_set_loss.csv", index=False, encoding="utf-8")
    #
    # cpu_tensor_list = [tensor.cpu() for tensor in sq_test_set_loss]
    # df = pd.DataFrame(cpu_tensor_list)
    #
    #
    # df.to_csv("test_set_loss.csv", index=False, encoding="utf-8")
    # df = pd.DataFrame(sq_val_set_err)
    #
    # df.to_csv("val_set_err.csv", index=False, encoding="utf-8")
    # df = pd.DataFrame(sq_train_set_ece)
    #
    # df.to_csv("train_set_ece.csv", index=False, encoding="utf-8")
    # df = pd.DataFrame(sq_val_set_ece)
    #
    # df.to_csv("val_set_ece.csv", index=False, encoding="utf-8")
