...
def variational_estimator(nn_class):
    ...
    def sample_elbo(self,
        ...
        for _ in range(sample_nbr):
            ...
            #时序模型校准
            if loss==0: res=outputs
            else: res=torch.cat((res,outputs),dim=1)
            loss += criterion(outputs, labels)
            #loss += self.nn_kl_divergence() * complexity_cost_weight
            '''

            #label=labels*(10*a-1)/9+(1-a)/9
            ...
            for i in range(len(outputs)):
                if i!=0:
                    ...
                    deg+=(outputs[i]-outputs[i-1])*(outputs[i]-outputs[i-1])
        ...
        #时序模型校准
        mean=torch.mean(res, dim=1)

        err=torch.abs(torch.flatten(mean)-torch.flatten(labels))

        std=torch.std(res, dim=1)*10


        loss2=torch.mean(torch.abs(err-std))
        # print('loss2')
        # print(loss2)
        # print('loss1')
        # print(loss)
        ...
        return (loss+loss2*10) / sample_nbr#时序
        #return loss/sample_nbr #诊断
    ...
    def sample_elbo_detailed_loss(self,
        ...
        print("************")
        print(np.array(y_hat), (likelihood_cost + complexity_cost) / sample_nbr,\
